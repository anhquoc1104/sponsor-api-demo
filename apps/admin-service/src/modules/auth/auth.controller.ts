import { ACCOUNT_TYPE, BaseController, Common, RESPONSE } from '@app/common';
...
export class AuthController extends BaseController {
  ...
  async authenticate(@Body() payload: BaseAuthenticateRequestDTO) {
    try {
      payload.type = ACCOUNT_TYPE.ADMIN;
  ...
  async getProfile(@User() user: IAccessTokenPayload) {
    try {
      ...
      if (Common.compareValues(response.type, ACCOUNT_TYPE.ADMIN)) {
        const menus: IPermissionFrame[] = [];
        PERMISSION_FRAME.map((element) => {
          ...
          if (matches?.length) {
            menus.push(element as IPermissionFrame);
          }
        });
