...
import { AttributePlatformService } from '@app/modules/sponsor-attribute/attribute-platform.service';
import { IAccessTokenPayload } from '@app/shared/jwt/interfaces/jwt.interface';
import { IAttributePlatform } from '@app/schemas';
import {
...
@Controller('attributes/platforms')
@ApiTags('Sponsor Platform')
export class AttributePlatformController extends BaseController {
  logger: Logger = new Logger(this.constructor.name);
  model_name = ENUM_MODEL.ATTRIBUTE_PLATFORM;

  constructor(private attributePlatformService: AttributePlatformService) {
    super();
  }

  @Get('')
  @ApiOperation({ summary: 'Danh sách platform' })
  ...
  async findAll(
    @Query() query: FindAttributePlatformDTO,
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Platform Detail' })
  @ApiParam({
    required: true,
    name: 'id',
    type: String,
  ...
  @Post('')
  @ApiOperation({ summary: 'Tạo platform' })
  @Roles(ACCOUNT_TYPE.ADMIN)
  @RequirePermission([PERMISSION.APPROVE_SPONSOR])
  @ApiResponse({
    status: 200,
  ...
  async create(
    @Body() body: CreateAttributePlatformDTO,
    @User() user: IAccessTokenPayload,
  ) {
    try {
      this.logger.log(
      ...
      const data: IAttributePlatform =
        await this.attributePlatformService.create(body);
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Chỉnh sửa platform' })
  @Roles(ACCOUNT_TYPE.ADMIN)
  @RequirePermission([PERMISSION.APPROVE_SPONSOR])
  @ApiResponse({
    status: 200,
  ...
  async update(
    ...
    @Body() body: UpdateAttributePlatformDTO,
    @User() user: IAccessTokenPayload,
  ) {
    try {
      this.logger.log(
      ...
      const data: IAttributePlatform =
        await this.attributePlatformService.update(id, body);
  ...
  @Put('status/:id')
  @ApiOperation({ summary: 'Change status platform' })
  @ApiParam({
    required: true,
    name: 'id',
    type: String,
  ...
  async updateStatus(
    @Param('id', new ValidateObjectIdPipe()) id,
    @Body() body: UpdateStatusAttributePlatformDTO,
    @User() user: any,
  ) {
    try {
      this.logger.log(
      ...
      await this.updateControllerByUser(body, user);
      const data: IAttributePlatform =
        await this.attributePlatformService.updateStatus(id, body);
